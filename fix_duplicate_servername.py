...
def main():
    ...
    try:
        ...
        # 创建Python脚本，修复server_name重复问题
        fix_script = '''
# 读取配置文件
with open('/etc/nginx/sites-enabled/nuotao', 'r') as f:
    content = f.read()

print("修复前的server_name行:")
for i, line in enumerate(content.split('\\n'), 1):
    if 'server_name' in line:
        print(f"  行{i}: {repr(line)}")

# 修复重复的server_name
# 把 "server_name admin.nuotaoutdoor.com admin.nuotaoutdoor.com;" 
# 改成 "server_name admin.nuotaoutdoor.com;"
old_server_name = 'server_name admin.nuotaoutdoor.com admin.nuotaoutdoor.com;'
new_server_name = 'server_name admin.nuotaoutdoor.com;'

print(f"\\n旧server_name出现次数: {content.count(old_server_name)}")
content = content.replace(old_server_name, new_server_name)
...
'''
        ...
        # 将Python脚本写入服务器并执行
        run_command(ssh, f"cat > /tmp/fix_duplicate.py << 'PYEOF'\n{fix_script}\nPYEOF")
        run_command(ssh, "python3 /tmp/fix_duplicate.py")
