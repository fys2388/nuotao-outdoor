...
const menuItems = [
  ...
  },
  {
    ...
    children: [
      ...
      { key: 'b2b', icon: <TeamOutlined />, label: 'B2B代理商', dynamicBadge: 'b2bPending' },
    ],
  },
  ...
  useEffect(() => {
    const fetchPending = async () => {
      ...
      try {
        ...
        const b2bRes = await fetch('/api/v1/admin/b2b/stats', { headers })
        if (b2bRes.ok) {
          const b2bData = await b2bRes.json()
          setB2bPendingCount(b2bData.pending_agents ?? 0)
        }
      } catch {
        // 静默失败
      }
      ...
      >
        ...
        <Menu
          ...
          items={menuItems.map(item => {
            const badgeCount = item.dynamicBadge === 'pending' ? pendingCount : (item.dynamicBadge === 'alerts' ? alertsCount : (item.dynamicBadge === 'b2bPending' ? b2bPendingCount : item.badge))
            const children = item.children?.map(child => {
              const childBadge = child.dynamicBadge === 'pending' ? pendingCount : (child.dynamicBadge === 'alerts' ? alertsCount : (child.dynamicBadge === 'b2bPending' ? b2bPendingCount : child.badge))
              return {
                ...child,
                label: childBadge ? (
                  <Space>
                    {child.label}
                    <Badge count={childBadge} size="small" />
                  </Space>
                ) : child.label,
              }
            })
