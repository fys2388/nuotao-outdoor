...
export default function ProductCandidatesPage() {
  ...
  const [scoreBatchLoading, setScoreBatchLoading] = useState(false)
  const [statusBatchLoading, setStatusBatchLoading] = useState<string | null>(null)
  ...
  const scoreBatch = async () => {
    const ids = selectedRowKeys
    if (!ids.length) {
      message.info('请先勾选要评分的候选商品')
      return
    }
    setScoreBatchLoading(true)
    let ok = 0
    const failed: string[] = []
    // 并发 4 条，避免一次性打爆后端
    const queue = [...ids]
    ...
    message.success(`六维评分完成 ${ok} 条` + (failed.length ? `，${failed.length} 条失败` : ''))
    setSelectedRowKeys([])
    setScoreBatchLoading(false)
    await loadCandidates()
  }

  const moveBatch = (status: CandidateStatus) => {
    const ids = selectedRowKeys
    if (!ids.length) {
      message.info('请先勾选候选商品')
      return
    }
    const label = STATUS_META[status].label
    Modal.confirm({
      title: `批量${label}（${ids.length} 条）`,
      content: '该动作会逐条写入候选状态机审计记录，且必须由人工操作。',
      okText: '确认',
      cancelText: '取消',
      onOk: async () => {
        setStatusBatchLoading(status)
        let ok = 0
        const failed: string[] = []
        const queue = [...ids]
        const workers = Array.from({ length: Math.min(4, queue.length) }, async () => {
          while (queue.length) {
            ...
            if (!id) break
            try {
              await api.updateProductCandidateStatus(id, {
                status,
        ...
        await Promise.all(workers)
        message.success(`已批量${label} ${ok} 条` + (failed.length ? `，${failed.length} 条失败` : ''))
        setSelectedRowKeys([])
        setStatusBatchLoading(null)
        await loadCandidates()
      },
    })
  }
  ...
  const deleteBatch = () => {
    const ids = selectedRowKeys
    if (!ids.length) {
      message.info('请先勾选要删除的候选商品')
      return
    }
    Modal.confirm({
      title: `批量删除（${ids.length} 条）`,
      content: `确定删除选中的 ${ids.length} 条候选商品吗？将执行软删除（从列表移除，数据库保留可恢复），此操作需人工确认。`,
      okText: '确认删除',
      okType: 'danger',
      cancelText: '取消',
      onOk: async () => {
        ...
        try {
          const res = (await api.batchDeleteProducts(ids)) as {
            deleted: number
            not_found?: string[]
          }
          const missing = res.not_found?.length || 0
          message.success(`已删除 ${res.deleted} 条` + (missing ? `，${missing} 条不存在` : ''))
          setSelectedRowKeys([])
          await loadCandidates()
        } catch (actionError) {
          message.error(`批量删除失败：${apiErrorMessage(actionError)}`)
          throw actionError
        } finally {
    ...
    <div className="resource-page">
      <div className="page-heading">
        ...
        <Space wrap>
          <Button
            icon={<ThunderboltOutlined />}
            loading={scoreBatchLoading}
            disabled={selectedRowKeys.length === 0}
            onClick={() => void scoreBatch()}
          >
            批量评分{selectedRowKeys.length ? `（${selectedRowKeys.length}）` : ''}
          </Button>
          <Button
            icon={<ExperimentOutlined />}
          ...
          <Button
            disabled={selectedRowKeys.length === 0 || statusBatchLoading !== null}
            loading={statusBatchLoading === 'approved'}
            onClick={() => moveBatch('approved')}
          >
          ...
          <Button
            danger
            disabled={selectedRowKeys.length === 0 || statusBatchLoading !== null}
            loading={statusBatchLoading === 'rejected'}
            onClick={() => moveBatch('rejected')}
          >
            批量淘汰{selectedRowKeys.length ? `（${selectedRowKeys.length}）` : ''}
          </Button>
          <Button
            danger
            ghost
      ...
      <Card variant="borderless" className="resource-panel">
        <div className="resource-toolbar">
          ...
          {selectedRowKeys.length > 0 ? (
            <Space size={4}>
              <Text type="secondary" style={{ fontSize: 12 }}>已选 {selectedRowKeys.length}</Text>
              <Button
                size="small"
                type="link"
                onClick={() => setSelectedRowKeys(filteredCandidates.map((c) => c.id))}
              >
                全选筛选结果
              </Button>
