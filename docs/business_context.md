...
Nuotao Outdoor 是 AI 驱动的跨境户外品牌，核心逻辑：**用中国供应链的成本优势 + AI 运营效率，同时服务海外消费者与企业客户**。

正式业务结构为 B2C + B2B 双轮驱动：

| 业务模式 | 主要客户 | 核心价值 |
|---|---|---|
| B2C | 消费者 | 品牌、流量、用户数据、产品验证、零售利润 |
| B2B | 零售商、批发商、代理商、分销商、小 B 客户 | 批量出货、渠道覆盖、库存消化、现金流 |

两种模式共享商品、供应商、采购、库存、WMS、TMS、CRM、Agent 和 BI 底座，但保留独立的价格、订单、履约和结算规则。
...
| 阶段 3 | 平台化 | 扩充海外代理商与 B2B 分销渠道 | 非 DTC 收入占比提升、渠道网络形成 |

> 架构调整（2026-09-13）：B2B 仍是业务规模化阶段，但数据库、权限和领域模型从 Phase 1 起即同时支持 B2C 与 B2B，避免后期重构。详见 `docs/business_decisions/ADR/COMMERCE-001.md`。
...
| v0.4 | 2026-09-13 | 正式定位 B2C + B2B 一体化经营操作系统，修正三阶段与共享底座描述 |
