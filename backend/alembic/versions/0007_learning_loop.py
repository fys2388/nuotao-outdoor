...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_confidence_calibration_workspace", "confidence_calibration", ["workspace_id"]
    )
    op.execute(
        sa.text(
    ...
    op.create_table(
        ...
        sa.Column(
            "current_weights", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
        sa.Column(
            "suggested_weights", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
    ...
    op.create_index(
        "ix_score_calibration_runs_workspace", "score_calibration_runs", ["workspace_id"]
    )
    op.execute(
        sa.text(
    ...
    op.create_index(
        "ix_product_knowledge_entries_workspace", "product_knowledge_entries", ["workspace_id"]
    )
    op.create_index(
        "ix_product_knowledge_entries_product", "product_knowledge_entries", ["product_id"]
    )
    op.create_index(
        "ix_product_knowledge_entries_type", "product_knowledge_entries", ["entry_type"]
    )
    op.execute(
        sa.text(
