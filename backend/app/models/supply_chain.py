...
# pending_cost_confirmation: cost missing or unconfirmed; cannot be approved/ordered.
...
class PurchaseOrder(Base, TimestampMixin, WorkspaceMixin):
    ...
    # --- Cost confirmation (M0-M4.1 production blocker) ---
    # cost_confirmed=True means real ProductCost exists and total_landed_cost is confirmed.
    # cost_source: product_cost (real) / manual (user entered) / estimate (fallback ratio, NOT for execution)
    # cost_confidence: 0.0-1.0, estimate must be < 0.5
    cost_confirmed: Mapped[bool] = mapped_column(nullable=False, default=False, index=True)
    cost_source: Mapped[str] = mapped_column(String(32), nullable=False, default="unknown")
    cost_confidence: Mapped[Decimal] = mapped_column(Numeric(4, 3), nullable=False, default=0)
    cost_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    cost_confirmed_by: Mapped[str | None] = mapped_column(String(128), nullable=True)
    cost_block_reason: Mapped[str | None] = mapped_column(String(500), nullable=True)
