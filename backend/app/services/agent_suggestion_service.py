    ...
    *,
    ...
    try:
        ...
        approval_result = await auto_approve_suggestion(
            session, suggestion, trace_id=None,
        )
        logger.info(
            "Agent自动审批结果: suggestion_id=%s decision=%s confidence=%s reviewer=%s auto_executed=%s",
            suggestion.id,
            approval_result.get("decision"),
            approval_result.get("confidence"),
            approval_result.get("reviewer"),
            approval_result.get("auto_executed"),
        )
