...
# pending_cost_confirmation -> draft (after cost confirmed) or cancelled.
# pending_cost_confirmation CANNOT go directly to approved/ordered.
PO_TRANSITIONS: dict[str, set[str]] = {
    "draft": {"approved", "cancelled", "pending_cost_confirmation"},
    "pending_cost_confirmation": {"draft", "cancelled"},
...
async def create_supplier(
    ...
    *,
    ...
    """Create a supplier master record (unique code per workspace)."""
    supplier = Supplier(
    ...
    try:
        ...
        raise SupplyChainError("supplier code already exists") from exc
    await event_service.create_event(
        ...
        payload={"code": data.code, "name": data.name, "platform": data.platform},
        trace_id=trace_id,
    )
    logger.info("supplier %s (%s) created trace=%s", supplier.id, data.code, trace_id)
    return supplier
...
async def get_supplier(
    ...
    """Load one supplier or raise 404."""
    supplier = await _load_supplier(session, workspace_id=workspace_id, supplier_id=supplier_id)
    if supplier is None:
        raise SupplyChainError("supplier not found")
    return supplier
...
async def list_suppliers(
    ...
    *,
    ...
    """List suppliers with optional filters, newest first."""
    query = select(Supplier).where(Supplier.workspace_id == workspace_id)
    if status:
    ...
    if search:
        pattern = f"%{search}%"
        query = query.where(Supplier.name.ilike(pattern) | Supplier.code.ilike(pattern))
    query = query.order_by(Supplier.created_at.desc()).limit(limit)
...
async def update_supplier(
    ...
    *,
    ...
    supplier = await get_supplier(session, workspace_id=workspace_id, supplier_id=supplier_id)
    changed: dict[str, Any] = {}
    for field in ("name", "platform", "shop_url", "rating", "status", "contact"):
    ...
    logger.info("supplier %s updated trace=%s", supplier.id, trace_id)
    return supplier
...
async def delete_supplier(
    ...
    *,
    ...
    """Delete a supplier (soft: set status=inactive; hard delete if no PO references)."""
    supplier = await get_supplier(session, workspace_id=workspace_id, supplier_id=supplier_id)
    # Check for purchase order references
    po_count = (
        await session.execute(
            select(PurchaseOrder.id).where(PurchaseOrder.supplier_id == supplier_id)
        )
    ).scalars().all()
    if po_count:
        ...
        await session.flush()
        logger.info("supplier %s soft-deleted (inactive) due to PO references trace=%s", supplier.id, trace_id)
    else:
        await session.delete(supplier)
        await session.flush()
        logger.info("supplier %s hard-deleted trace=%s", supplier.id, trace_id)
    await event_service.create_event(
        session,
        workspace_id=workspace_id,
        event_type="supply.supplier_deleted",
        entity_type="supplier",
        entity_id=str(supplier.id),
        payload={"soft": bool(po_count)},
        trace_id=trace_id,
    )
